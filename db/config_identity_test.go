...
func TestNewConfigItemFromResultDualWriteIdentityFields(t *testing.T) {
	...
	result := v1.ScrapeResult{
		ID:          " AWS/Resource-123 ",
		Aliases:     []string{"alias-one", "ALIAS-ONE", "", "aws/resource-123", " alias-two ", "ALIAS-TWO"},
	...
	assert.Equal(t, []string{"alias-one", "alias-two"}, []string(ci.Aliases))
	assert.Equal(t, []string{"aws/resource-123", "alias-one", "alias-two"}, []string(ci.ExternalID))
}
