...
func TestAWSS3TemporaryTableSQL(t *testing.T) {
	...
	want := "CREATE TEMPORARY TABLE cloudtrail AS s3('https://cloudtrail.s3.us-east-1.amazonaws.com/AWSLogs/123/*.json.gz','ASIA','sec''ret','token','JSONAsString','json String','gzip');"
	if cmd != want {
...
func TestAWSS3TemporaryTableSQLDefaults(t *testing.T) {
	...
	want := "CREATE TEMPORARY TABLE cloudtrail AS s3('https://cloudtrail.s3.amazonaws.com','AKIA','secret','JSONAsString','json String');"
	if cmd != want {
		t.Fatalf("expected %q, got %q", want, cmd)
	}
}
