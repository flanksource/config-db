...
var _ = Describe("Scrapers test", Ordered, func() {
	...
	Describe("Test kubernetes relationship", func() {
		...
		It("should have cluster and namespace tags on config items", func() {
			var configItems []models.ConfigItem
			err := DefaultContext.DB().
				Where("name IN (?, ?, ?)", "first-config", "second-config", "first-secret").Find(&configItems).Error
			Expect(err).To(BeNil())
			Expect(configItems).To(HaveLen(3))

			for _, c := range configItems {
				Expect(c.Tags).To(HaveKeyWithValue("cluster", scrapeConfig.Spec.Kubernetes[0].ClusterName),
					"config %s should have cluster tag", lo.FromPtr(c.Name))
				Expect(c.Tags).To(HaveKeyWithValue("namespace", "default"),
					"config %s should have namespace tag", lo.FromPtr(c.Name))
			}
		})
