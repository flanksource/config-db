...
var _ = Describe("DecodeDescriptor", func() {
	DescribeTable("decodes every descriptor form to its canonical primitive",
		...
		Entry("ClaimsIdentity descriptor → email",
			`Microsoft.IdentityModel.Claims.ClaimsIdentity;00691924-e082-4301-a3dc-1732afd14289\mimmerman@example.com`,
			"",
			DecodedDescriptor{
				Form:  "claims",
				Email: "mimmerman@example.com",
			}),
		...
		Entry("TF ServiceIdentity descriptor → labelled service + payload",
			...
			DecodedDescriptor{
				Form:           "service",
				ServiceLabel:   "Build Service (ExampleApp)",
...
var _ = Describe("DescriptorAliases", func() {
	It("a ClaimsIdentity descriptor reduces to just the email", func() {
		descriptor := `Microsoft.IdentityModel.Claims.ClaimsIdentity;00691924-e082-4301-a3dc-1732afd14289\mimmerman@example.com`
		Expect(DescriptorAliases(descriptor)).To(Equal([]string{"mimmerman@example.com"}))
	})
...
var _ = Describe("ResolvedIdentityName", func() {
	It("uses the projectName label for ServiceIdentity descriptors", func() {
		...
		Expect(ResolvedIdentityName(id, "ExampleApp")).To(Equal("Build Service (ExampleApp)"))
	})

	It("falls through to ProviderDisplayName for ClaimsIdentity (caller already has the email)", func() {
		id := ResolvedIdentity{
			Descriptor:          `Microsoft.IdentityModel.Claims.ClaimsIdentity;00691924-e082-4301-a3dc-1732afd14289\mimmerman@example.com`,
			ProviderDisplayName: "Example User",
		}
		Expect(ResolvedIdentityName(id, "")).To(Equal("Example User"))
	})
})
