...
type runCache struct {
	...
	pipelineLastLoad map[string]time.Time
}
...
func (c *runCache) ensureFresh(ctx api.ScrapeContext, ttl time.Duration, project string, pipelineID int) error {
	...
	stale := !loaded || time.Since(lastLoad) >= ttl
	c.RUnlock()
...
func (c *runCache) load(ctx api.ScrapeContext, pattern string, pipelineKey string) error {
	if ctx.DB() == nil {
		return nil
	}
	...
	if c.pipelineLastLoad == nil {
		c.pipelineLastLoad = make(map[string]time.Time)
	}
	for _, row := range rows {
		c.ids[row.ExternalChangeID] = struct{}{}
	}
	c.pipelineLastLoad[pipelineKey] = time.Now()
	c.Unlock()

	ctx.Logger.V(4).Infof("terminal-run cache loaded %d entries for pattern %s", len(rows), pattern)
	return nil
}
...
// defCacheKey is a composite key for pipeline definition caching that avoids
// cross-project ID collisions when multiple orgs/projects are scraped.
...
type defCache struct {
	sync.RWMutex
	entries map[defCacheKey]cachedPipelineDef
}

// get returns a cached definition if the revision matches.
func (c *defCache) get(org, project string, pipelineID, revision int) (*PipelineDefinition, bool) {
	c.RLock()
	defer c.RUnlock()
	key := defCacheKey{Organization: org, Project: project, PipelineID: pipelineID}
...
func (c *defCache) set(org, project string, pipelineID, revision int, def *PipelineDefinition) {
	c.Lock()
	defer c.Unlock()
	if c.entries == nil {
		c.entries = make(map[defCacheKey]cachedPipelineDef)
	}
	key := defCacheKey{Organization: org, Project: project, PipelineID: pipelineID}
	c.entries[key] = cachedPipelineDef{Revision: revision, Definition: def}
}
