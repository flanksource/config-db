...
func TestRunCacheTTLExpiry(t *testing.T) {
	...
	staleCache := &runCache{pipelineLastLoad: map[string]time.Time{pipelineKey: time.Now().Add(-2 * time.Hour)}}
	staleCache.ids = map[string]struct{}{"stale/1/1": {}}
	if !staleCache.has("stale/1/1") {
		t.Fatal("pre-seeded ID must be present")
	}
	staleCache.RLock()
	isStale := time.Since(staleCache.pipelineLastLoad[pipelineKey]) >= time.Hour
	staleCache.RUnlock()
	if !isStale {
	...
	freshCache := &runCache{pipelineLastLoad: map[string]time.Time{freshKey: time.Now().Add(-30 * time.Minute)}}
	freshCache.ids = map[string]struct{}{"fresh/1/1": {}}
	if !freshCache.has("fresh/1/1") {
		t.Fatal("pre-seeded ID must be present")
	}
	freshCache.RLock()
	isFresh := time.Since(freshCache.pipelineLastLoad[freshKey]) < time.Hour
	freshCache.RUnlock()
	if !isFresh {
...
func TestPipelineDefCacheHitMiss(t *testing.T) {
	...
	// Miss on empty cache
	if got, ok := c.get(org, project, pipelineID, revision); ok || got != nil {
		t.Fatal("empty cache should return miss")
	}

	def := &PipelineDefinition{YamlPath: "build.yaml"}
	c.set(org, project, pipelineID, revision, def)

	// Hit with matching revision
	got, ok := c.get(org, project, pipelineID, revision)
	if !ok || got != def {
		t.Fatal("expected cache hit with matching revision")
	}

	// Miss with different revision
	if got, ok := c.get(org, project, pipelineID, revision+1); ok || got != nil {
		t.Fatal("expected cache miss for different revision")
	}

	// Miss with different org (same pipelineID)
	if got, ok := c.get("otherorg", project, pipelineID, revision); ok || got != nil {
		t.Fatal("expected cache miss for different org")
	}

	// Miss with different project (same pipelineID)
	if got, ok := c.get(org, "otherproject", pipelineID, revision); ok || got != nil {
		t.Fatal("expected cache miss for different project")
	}
}
...
func TestPipelineDefCacheRevisionUpdate(t *testing.T) {
	...
	c.set(org, project, pipelineID, 1, def1)
	c.set(org, project, pipelineID, 2, def2)

	// Only revision 2 should be present
	got, ok := c.get(org, project, pipelineID, 2)
	if !ok || got != def2 {
		t.Fatal("expected updated definition after revision bump")
	}
	if _, ok := c.get(org, project, pipelineID, 1); ok {
...
func TestPipelineGetIDStable(t *testing.T) {
	...
	webHrefStripped := "https://dev.azure.com/myorg/myproject/_build/definition"
	apiURL := "https://dev.azure.com/myorg/myproject/_apis/pipelines/42?revision=7"

	// Web link present — prefer it, query params stripped for stability
	p := Pipeline{URL: apiURL, Links: map[string]Link{"web": {Href: webHref}}}
	if got := p.GetID(); got != webHrefStripped {
		t.Errorf("expected stripped web href %q, got %q", webHrefStripped, got)
	}

	// Web link without query params — returned as-is
	cleanWebHref := "https://dev.azure.com/myorg/myproject/_build/definition"
	pClean := Pipeline{URL: apiURL, Links: map[string]Link{"web": {Href: cleanWebHref}}}
	if got := pClean.GetID(); got != cleanWebHref {
		t.Errorf("expected clean web href %q, got %q", cleanWebHref, got)
	}
