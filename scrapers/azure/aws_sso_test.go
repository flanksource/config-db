...
var _ = Describe("parseAWSSSOAppRoleValue", func() {
	DescribeTable("returns expected (roleARN, samlARN, ok) for AppRole value strings",
		...
		Entry("customer AWS SSO app role value",
			"arn:aws:iam::111111111111:role/AWSManagedServicesAdminRole,arn:aws:iam::111111111111:saml-provider/customer-saml",
			"arn:aws:iam::111111111111:role/AWSManagedServicesAdminRole",
			"arn:aws:iam::111111111111:saml-provider/customer-saml",
			true),
...
var _ = Describe("awsIAMRoleName", func() {
	DescribeTable("returns the IAM role name from an ARN",
		...
		Entry("path-qualified role ARN",
			"arn:aws:iam::111111111111:role/service-role/AWSManagedServicesAdminRole",
			"AWSManagedServicesAdminRole"),
...
var _ = Describe("appendAWSSSOAssignment", func() {
	It("scopes config access to the AWS account and uses the IAM role as the external role", func() {
		...
		link := awsSSOLink{
			...
			RoleARN:   "arn:aws:iam::111111111111:role/AWSManagedServicesAdminRole",
			SAMLARN:   "arn:aws:iam::111111111111:saml-provider/customer-saml",
		}

		var result v1.ScrapeResult
		emitted := map[string]struct{}{}
		appendAWSSSOAssignment(&result, v1.ExternalConfigAccess{OwnerScraperID: &scraperID, ExternalUserID: &principalID}, "assignment-1", spID, link, "AWSExampleSharedInsuranceNonProdAdminRole", &scraperID, emitted)
		appendAWSSSOAssignment(&result, v1.ExternalConfigAccess{OwnerScraperID: &scraperID, ExternalUserID: &principalID}, "assignment-2", spID, link, "AWSExampleSharedInsuranceNonProdAdminRole", &scraperID, emitted)

		Expect(result.ExternalRoles).To(HaveLen(1))
		Expect(result.ExternalRoles[0].Tenant).To(Equal("111111111111"))
		...
		for _, access := range result.ConfigAccess {
			...
			Expect(access.ConfigExternalID.ExternalID).To(Equal("111111111111"))
