...
func (azure Scraper) hydrateConnection(ctx api.ScrapeContext, t v1.Azure) (v1.Azure, connection.AzureConnection, error) {
	...
	if t.ConnectionName != "" {
		...
		if azureConn.ClientID != nil {
			t.ClientID = *azureConn.ClientID
		}
		if azureConn.ClientSecret != nil {
	...
	for i, r := range results {
		...
		if relateSubscription {
			results[i].RelationshipResults = append(results[i].RelationshipResults, v1.RelationshipResult{
				ConfigExternalID:  v1.ExternalID{ExternalID: "/subscriptions/" + azure.config.SubscriptionID, ConfigType: ConfigTypePrefix + "SUBSCRIPTION"},
		...
		if relateResourceGroup && extractResourceGroup(r.ID) != "" {
			results[i].RelationshipResults = append(results[i].RelationshipResults, v1.RelationshipResult{
				...
				ConfigExternalID: v1.ExternalID{
					...
					ExternalID: fmt.Sprintf("/subscriptions/%s/resourcegroups/%s", azure.config.SubscriptionID, extractResourceGroup(r.ID)),
				},
