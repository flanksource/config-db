...
var _ = ginkgo.Describe("Logs Scraper - OpenSearch", ginkgo.Ordered, func() {
	...
	ginkgo.BeforeAll(func() {
				...
				SQL: []v1.SQL{
					{
						Connection: v1.Connection{
							Connection: "mysql://localhost:5432/test",
						},
						Query: "SELECT 'mysql-0' as name",
					},
				},
			},
		}
		...
		configItemID := uuid.MustParse("45c381f0-7f73-40d3-88f3-5653a7c518d1")
		configItem = dbmodels.ConfigItem{
			ID:         configItemID.String(),
			ExternalID: []string{configItemID.String(), "mysql-0"},
			Type:       "Database::MySQL",
			Name:       lo.ToPtr("mysql-0"),
			Config:     lo.ToPtr(`{"name":"mysql-0","port":5432,"host":"localhost"}`),
